...
import { Search, CheckCircle, XCircle, ShieldCheck, ChevronLeft, ChevronRight, AlertCircle } from 'lucide-react';
...
const AdminCompanies = () => {
  ...
  const handleVerify = async (id, currentVerified) => {
    ...
    try {
      await verifyCompany(id, !currentVerified);
      await fetchCompanies();
    } catch (err) {
      alert(err?.response?.data?.message || 'Action failed.');
    } finally {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-[color:var(--admin-text-secondary)] text-sm mt-1">{data.total} total companies in registry</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <select
          ...
          className="px-4 py-2 bg-[color:var(--admin-bg-card)] border border-[color:var(--admin-border)] rounded-lg text-sm text-[color:var(--admin-text-secondary)] focus:outline-none focus:border-blue-500"
        >
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-800">
              {loading ? (
                <tr><td colSpan={6} className="py-16 text-center text-[color:var(--admin-text-secondary)]">
                ...
                <tr key={company._id} className="hover:bg-[color:var(--admin-hover)]/30 transition-colors">
                  <td className="px-5 py-3.5">
                    <p className="text-[color:var(--admin-text-primary)] font-medium">{company.companyName}</p>
                    <p className="text-xs text-[color:var(--admin-text-secondary)] mt-0.5">{company.address?.country || '—'}</p>
                  </td>
                  <td className="px-5 py-3.5 text-[color:var(--admin-text-secondary)] hidden sm:table-cell">{company.industryType || '—'}</td>
                  <td className="px-5 py-3.5"><Badge status={company.status} /></td>
                  <td className="px-5 py-3.5 hidden md:table-cell">
                    {company.isVerified
                      ? <CheckCircle className="h-4 w-4 text-emerald-400" />
                      : <XCircle className="h-4 w-4 text-slate-600" />}
                  </td>
                  <td className="px-5 py-3.5 text-[color:var(--admin-text-secondary)] text-xs hidden lg:table-cell">
                    ...
                    <div className="flex items-center justify-end gap-1 flex-wrap">
                      {company.status !== 'active' && (
                        <button
                          ...
                          className="px-2 py-1 text-xs font-semibold text-emerald-400 border border-emerald-500/30 rounded hover:bg-emerald-500/10 transition-colors disabled:opacity-40"
                        >Approve</button>
                      )}
                      {company.status !== 'rejected' && (
                        <button
                          ...
                          disabled={!!actionLoading}
                          className="px-2 py-1 text-xs font-semibold text-red-400 border border-red-500/30 rounded hover:bg-red-500/10 transition-colors disabled:opacity-40"
                        >Reject</button>
                      )}
                      {company.status !== 'suspended' && (
                        <button
                          ...
                          disabled={!!actionLoading}
                          className="px-2 py-1 text-xs font-semibold text-amber-400 border border-amber-500/30 rounded hover:bg-amber-500/10 transition-colors disabled:opacity-40"
                        >Suspend</button>
                      )}
                      <button
                        onClick={() => handleVerify(company._id, company.isVerified)}
                        disabled={!!actionLoading}
                        className="px-2 py-1 text-xs font-semibold text-blue-400 border border-blue-500/30 rounded hover:bg-blue-500/10 transition-colors disabled:opacity-40 flex items-center gap-1"
                      >
          ...
          <div className="flex items-center justify-between px-5 py-3 border-t border-[color:var(--admin-border)]">
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                disabled={page === 1}
                className="p-1.5 rounded text-[color:var(--admin-text-secondary)] hover:text-[color:var(--admin-text-primary)] hover:bg-[color:var(--admin-hover)] disabled:opacity-30 transition-colors"
              ><ChevronLeft className="h-4 w-4" /></button>

              {getPages().map((p, i) =>
                p === '...'
                  ? <span key={i} className="px-2 text-[color:var(--admin-text-secondary)] text-sm">…</span>
                  : <button
                      key={p}
                      onClick={() => setPage(p)}
                      className={`w-8 h-8 rounded text-sm font-medium transition-colors ${p === page ? 'bg-blue-600 text-[color:var(--admin-text-primary)]' : 'text-[color:var(--admin-text-secondary)] hover:text-[color:var(--admin-text-primary)] hover:bg-[color:var(--admin-hover)]'}`}
                    >{p}</button>
              )}
