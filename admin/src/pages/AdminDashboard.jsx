...
import { Building2, Users, Eye, ClipboardList, Star, TrendingUp, AlertCircle } from 'lucide-react';
...
const StatCard = ({ icon: Icon, label, value, color, sub }) => (
  <div className="bg-[color:var(--admin-bg-card)] border border-[color:var(--admin-border)] shadow-sm rounded-2xl p-5 flex items-start gap-4 hover:border-[color:var(--admin-border)] transition-colors">
    <div className={`h-11 w-11 rounded-xl flex items-center justify-center flex-shrink-0 ${color} shadow-lg`}>
    ...
    <div className="space-y-8">
      ...
      <div className="bg-[color:var(--admin-bg-card)] border border-[color:var(--admin-border)] shadow-sm rounded-2xl p-6">
        <div className="flex items-center gap-2 mb-6">
          ...
          <span className="text-xs text-[color:var(--admin-text-secondary)] ml-auto">Last 30 data points</span>
        </div>
        {growth.length === 0 ? (
          <p className="text-[color:var(--admin-text-secondary)] text-sm text-center py-16">No analytics data yet.</p>
        ) : (
