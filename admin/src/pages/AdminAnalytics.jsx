...
const SummaryCard = ({ label, value, color }) => (
  <div className="bg-[color:var(--admin-bg-card)] border border-[color:var(--admin-border)] shadow-sm rounded-2xl p-5 hover:border-[color:var(--admin-border)] transition-colors">
...
const AdminAnalytics = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try { setGrowth(await getPlatformGrowth()); }
  ...
  const totals = growth.reduce(
    (acc, d) => ({ views: acc.views + (d.views || 0), inquiries: acc.inquiries + (d.inquiries || 0), contactClicks: acc.contactClicks + (d.contactClicks || 0) }),
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <SummaryCard label="Total Views"          value={totals.views}         color="text-blue-400" />
        <SummaryCard label="Total Inquiries"      value={totals.inquiries}     color="text-emerald-400" />
        <SummaryCard label="Contact Clicks"       value={totals.contactClicks} color="text-amber-400" />
      </div>

      <div className="bg-[color:var(--admin-bg-card)] border border-[color:var(--admin-border)] shadow-sm rounded-2xl p-6">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-slate-600">
            ...
            <p className="font-medium text-[color:var(--admin-text-secondary)]">No analytics data yet</p>
            <p className="text-sm mt-1">Data appears as users interact with listings.</p>
          </div>
        ) : (
          <ResponsiveContainer width="100%" height={340}>
            <LineChart data={growth} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
              ...
              <Tooltip contentStyle={{ backgroundColor: 'var(--admin-bg-card)', border: '1px solid var(--admin-border)', borderRadius: 12 }} labelStyle={{ color: '#94a3b8' }} itemStyle={{ color: '#e2e8f0' }} />
