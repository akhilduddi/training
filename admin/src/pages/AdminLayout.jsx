...
import {
  LayoutDashboard, Building2, Users, Star, BarChart2, LogOut, Menu, ShieldCheck, Sun, Moon, Monitor, Briefcase, FileText
} from 'lucide-react';
  ...
  const Sidebar = () => (
    <aside className="flex flex-col h-full w-64 flex-shrink-0 bg-[color:var(--admin-bg-nav)] border-r border-[color:var(--admin-border)]">
      ...
      <nav className="flex-1 px-3 py-6 space-y-1 overflow-y-auto">
        {getNavLinks(user?.role).map(({ to, label, Icon, end }) => (
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="px-4 lg:px-8 py-3 flex items-center gap-4 flex-shrink-0 bg-[color:var(--admin-bg-nav)] border-b border-[color:var(--admin-border)]">
          ...
          <div className="ml-auto flex items-center gap-4">
            ...
            <span className="hidden sm:inline-block text-xs text-blue-600 font-semibold border border-blue-500/30 bg-blue-500/10 px-2 py-1 rounded uppercase tracking-wider">
              {user?.role || 'Admin'}
            </span>
          </div>
        </header>
